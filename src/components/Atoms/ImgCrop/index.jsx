...
const ImgCrop = forwardRef((props, ref) => {
  ...
  const defaultVal = {
    zoomVal: 1, rotateVal: 0,
    x: 0, y: 0,
    width: 0, height: 0
  }

  const [crop, setCrop] = useState({ x: defaultVal.x, y: defaultVal.y });
  const [cropSize, setCropSize] = useState({ width: defaultVal.width, height: defaultVal.height });
  const [zoomVal, setZoomVal] = useState(defaultVal.zoomVal);
  const [rotateVal, setRotateVal] = useState(defaultVal.rotateVal);

  const mediaSizeRef = useRef({ width: 0, height: 0, naturalWidth: 0, naturalHeight: 0 });
  ...
  const _initMedia = (mediaSize) => {
    const mediaContainer = document.getElementsByClassName(`${pkg}-container`)[0];
    ...
    if (containerWidth > containerHeight) {
      const containerRatioHeight = containerWidth / aspect;
      adaptHeight = containerRatioHeight > containerHeight;
      if (adaptHeight) {
        zoomVal = containerHeight / containerRatioHeight;
      }
    } else {
      const containerRatioWidth = containerHeight * aspect;
      adaptHeight = containerRatioWidth <= containerWidth;
      if (!adaptHeight) {
        zoomVal = containerWidth / containerRatioWidth;
      }
    }
    ...
    if (adaptHeight) {
      cropWidth = containerHeight * aspect;
      cropHeight = containerHeight;
    } else {
      cropWidth = containerWidth;
      cropHeight = containerWidth / aspect;
    }

    const zoomWidth = cropWidth / width;
    const zoomHeight = cropHeight / height;
    ...
    setRotateVal(defaultVal.rotateVal);
    setCrop({ x: defaultVal.x, y: defaultVal.y });
    setZoomVal(zoomVal);
    setTimeout(() => {
  ...
  const onMediaLoaded = useCallback(_initMedia,
    [aspect]
  );
  ...
  const _reset = () => {
    _initMedia(mediaSizeRef.current);
  };
  ...
  const onOk = useCallback(async () => {
    ...
    const { x, y } = cropPixelsRef.current;
    let { width, height } = croppedAreaRef.current;
    ...
    <>
      ...
      {src && (
        ...
        >
          ...
          <div className={`${pkg}-control reset`}>
            <MyButton type="link"
              onClick={onReset}
            >
